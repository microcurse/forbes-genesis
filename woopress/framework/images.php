...
if ( !function_exists('vt_resize') ) {
	function vt_resize( $attach_id = null, $img_url = null, $width, $height, $crop = false) {
		...
		// this is not an attachment, let's use the image url
			...
			$file_path = get_attached_file( attachment_url_to_postid( $img_url ) );
			if(is_multisite()) $file_path = et_trim_multisite_folder($file_path);

			$image_src = wp_get_attachment_image_src( attachment_url_to_postid( $img_url ), 'full' );
		}
...
if(!function_exists('et_trim_multisite_folder')) {
	...
	function et_trim_multisite_folder($path) {
		...
		if ( isset($uploads_path[0]) && isset($path[1]) ) {
			return $uploads_path[0] . '/' . $path[1];
		}

		if ( isset($uploads_path[0]) && isset($uploads_path[1]) ) {
			return $uploads_path[0] . '/' . $uploads_path[1];
		}
