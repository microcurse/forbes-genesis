...
if ( ! class_exists( 'Envato_Theme_Setup_Wizard' ) ) {
	...
	class Envato_Theme_Setup_Wizard {
		...
		public function init_globals() {
            ...
            $this->versions = require apply_filters('etheme_file_url', ETHEME_CODE_DIR . '/theme-versions.php');
		...
		public function admin_menus() {
			...
			if ( $this->is_submenu_page() ) {
				...
				$add_subpage_function( $this->parent_slug, esc_html__( 'Setup Wizard', 'woopress' ), esc_html__( 'Setup Wizard', 'woopress' ), 'manage_options', $this->page_slug, array(
					$this,
					'setup_wizard',
				) );
			} else {
				add_theme_page( esc_html__( 'Setup Wizard', 'woopress' ), esc_html__( 'Setup Wizard', 'woopress' ), 'manage_options', $this->page_slug, array(
					$this,
					'setup_wizard',
				) );
			}
		...
		public function init_wizard_steps() {
			...
			$this->steps = array(
				'introduction' => array(
					'name'    => esc_html__( 'Introduction', 'woopress' ),
			...
			$this->steps['help_support']    = array(
				'name'    => esc_html__( 'Support', 'woopress' ),
				'view'    => array( $this, 'envato_setup_help_support' ),
				'handler' => '',
			);
			$this->steps['next_steps']      = array(
				'name'    => esc_html__( 'Ready!', 'woopress' ),
		...
		public function setup_wizard() {
			...
			wp_localize_script( 'envato-setup', 'envato_setup_params', array(
				...
				'verify_text'      => esc_html__( '...verifying', 'woopress' ),
			) );
		...
		public function setup_wizard_header() {
			...
			<head>
				...
				echo '<t';
				echo 'itle>' . esc_html__( 'Theme &rsaquo; Setup Wizard', 'woopress' ) . '</ti' . 'tle>'; ?>
			...
			<?php if ( 'next_steps' === $this->step ) : ?>
				<a class="wc-return-to-dashboard"
				   href="<?php echo esc_url( admin_url() ); ?>"><?php esc_html_e( 'Return to the WordPress Dashboard', 'woopress' ); ?></a>
			<?php endif; ?>
		...
		public function envato_setup_introduction() {
			...
			if ( false && isset( $_REQUEST['debug'] ) ) {
				...
				<h1><?php printf( esc_html__( 'Welcome to the setup wizard for %s.', 'woopress' ), wp_get_theme() ); ?></h1>
				<p><?php esc_html_e( 'It looks like you have already run the setup wizard. Below are some options: ', 'woopress' ); ?></p>
				<ul>
					<li>
						<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
						   class="button-primary button button-next button-large"><?php esc_html_e( 'Run Setup Wizard Again', 'woopress' ); ?></a>
					</li>
			<!-- 		<li>
						<form method="post">
							<input type="hidden" name="reset-font-defaults" value="yes">
							<input type="submit" class="button-primary button button-large button-next"
							       value="<?php esc_attr_e( 'Reset font style and colors', 'woopress' ); ?>" name="save_step"/>
...
Go to Theme Options and Activate theme with your Purchase code.</p>
				<p class="envato-setup-actions step">
					<a href="<?php echo esc_url( wp_get_referer() && ! strpos( wp_get_referer(), 'update.php' ) ? wp_get_referer() : admin_url( '' ) ); ?>"
					   class="button button-large"><?php esc_html_e( 'Cancel', 'woopress' ); ?></a>
				</p>
				<?php
			} else {
				?>
				...
				<img src="<?php echo ETHEME_CODE_IMAGES_URL; ?>/big-eight.png" alt="eight theme" class="elogo">
				<h1><?php printf( esc_html__( 'Welcome to WooPress Setup Wizard', 'woopress' ), wp_get_theme() ); ?></h1>
				<p><?php printf( esc_html__( 'Thank you for choosing our WooPress template.', 'woopress' ), wp_get_theme() ); ?></p>
				<p><?php printf( esc_html__( 'This setup wizard will help you to refresh and configure your website with a new layout. You will have Child Theme, Content, and Plugins installed in 5-10 minutes (depending on your server configuration). ', 'woopress' ), wp_get_theme() ); ?></p>
				<p><?php esc_html_e( 'No time right now? If you do not want to go through the wizard, you can skip, and get back to WordPress dashboard. Come back any time to continue!', 'woopress' ); ?></p>
				<p style="border:2px solid #ff4242; color:#c33434; padding:10px;">Important! If you need to update WooPress theme, just Cancel the setup wizard.

Go to Theme Options and Activate theme with your Purchase code.</p>
				<p class="envato-setup-actions step">
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
					   class="et-button button-active button-next"><?php esc_html_e( 'Let\'s Go!', 'woopress' ); ?></a>
					<a href="<?php echo esc_url( wp_get_referer() && ! strpos( wp_get_referer(), 'update.php' ) ? wp_get_referer() : admin_url( '' ) ); ?>"
					   class="et-button"><?php esc_html_e( 'Not right now', 'woopress' ); ?></a>
				</p>
				<?php
			}
		}
		...
		public function envato_setup_introduction_save() {
			...
			if ( ! empty( $_POST['reset-font-defaults'] ) && $_POST['reset-font-defaults'] == 'yes' ) {
				...
				?>
				<p>
					<strong><?php esc_html_e( 'Options have been reset. Please go to Appearance > Customize in the WordPress backend.', 'woopress' ); ?></strong>
				</p>
				<?php
				return true;
			}
		...
		private function _get_plugins( $version = false ) {
			...
			$core = array(
				'woopress-core' => $instance->plugins['woopress-core'],
			);

			// $revslider = array(
			// 	'revslider' => $instance->plugins['revslider'],
			// );

			unset ( $instance->plugins['woopress-core'] );
			// unset ( $instance->plugins['revslider'] );

			$instance->plugins = array_merge($core, $instance->plugins);
			// $instance->plugins = array_merge($instance->plugins, $revslider);
		...
		public function envato_setup_default_plugins() {
			...
			?>
			<h1><?php esc_html_e( 'Default Plugins', 'woopress' ); ?></h1>
			<form method="post" class="plugins-form" data-version="<?php echo esc_attr( $version_import ); ?>" data-home_id="<?php echo esc_attr( $home_id ); ?>">
				...
				if ( count( $plugins['all'] ) ) {
					?>
					<p><?php esc_html_e( 'Your website requires some additional plugins.  The following plugins will be installed:', 'woopress' ); ?></p>
					...
					echo '<p><strong>' . esc_html_e( 'Good news! All plugins are already installed and up to date. Please continue.', 'woopress' ) . '</strong></p>';
				} ?>

				<p><?php esc_html_e( 'Please, note that every external plugin can affect your website loading speed. You can add and remove plugins later on from within WordPress.', 'woopress' ); ?></p>
				...
				<p class="envato-setup-actions step">
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
					   ...
					   data-callback="install_plugins"><?php esc_html_e( 'Continue', 'woopress' ); ?></a>
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
					   class="et-button"><?php esc_html_e( 'Skip this step', 'woopress' ); ?></a>
					<?php wp_nonce_field( 'envato-setup' ); ?>
				</p>
			</form>
			<?php
		...
		public function ajax_plugins() {
			if ( ! check_ajax_referer( 'envato_setup_nonce', 'wpnonce' ) || empty( $_POST['slug'] ) ) {
				wp_send_json_error( array( 'error' => 1, 'message' => esc_html__( 'No Slug Found', 'woopress' ) ) );
			}
			$json = array();
			...
			foreach ( $plugins['activate'] as $slug => $plugin ) {
				if ( $_POST['slug'] == $slug ) {
					$json = array(
						...
						'action2'       => - 1,
						'message'       => esc_html__( 'Activating Plugin', 'woopress' ),
					);
					break;
				}
			}
			foreach ( $plugins['update'] as $slug => $plugin ) {
				if ( $_POST['slug'] == $slug ) {
					$json = array(
						...
						'action2'       => - 1,
						'message'       => esc_html__( 'Updating Plugin', 'woopress' ),
					);
					break;
				}
			}
			foreach ( $plugins['install'] as $slug => $plugin ) {
				if ( $_POST['slug'] == $slug ) {
					$json = array(
						...
						'action2'       => - 1,
						'message'       => esc_html__( 'Installing Plugin', 'woopress' ),
					);
					break;
				}
			}
			...
			if ( $json ) {
				...
				wp_send_json( array( 'done' => 1, 'message' => esc_html__( 'Success', 'woopress' ) ) );
			}
			exit;
		...
		public function envato_setup_default_content() {
			...
			?>
			<h1><?php esc_html_e( 'Default Content', 'woopress' ); ?></h1>
			<form method="post">
...
Once imported, this content can be managed from the WordPress dashboard.  </p>
	            ...
	            <div class="import-demos-wrapper">
	                <h3><?php esc_html_e( 'Import demo versions', 'woopress'); ?></h3>
	                <div class="import-demos">
                        <div class="version-preview active-version version-preview-default" data-version="default">
                            <div class="version-screenshot">
                                <img src="<?php echo ETHEME_CODE_IMAGES_URL.'/vers/v_default.jpg'; ?>" alt="v_default">
                                <a href="#" class="et-button button-import-version button-import-version" data-version="default">
                                    <?php echo esc_html__('Import demo', 'woopress'); ?>
                                </a>
                            </div>
                            <span class="version-title">Default content</span>
                        </div>
                        <?php if ( isset( $this->versions['default'] ) ) { unset( $this->versions['default']); } ?>
				...
				<p class="envato-setup-actions step">
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
					   class="et-button button-active button-next"
					   data-callback="install_content"><?php esc_html_e( 'Continue', 'woopress' ); ?></a>
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
					   class="et-button"><?php esc_html_e( 'Skip this step', 'woopress' ); ?></a>
					<?php wp_nonce_field( 'envato-setup' ); ?>
				</p>
		...
		public function envato_setup_logo_design() {
			...
			?>
			<h1><?php esc_html_e( 'Logo &amp; Design', 'woopress' ); ?></h1>
			<form method="post">
				<p><?php printf( esc_html__( 'Please add your logo below. For best results, the logo should be a transparent PNG ( 466 by 277 pixels). The logo can be changed at any time from the Appearance > Customize area in your dashboard. Try %sEnvato Studio%s if you need a new logo designed.', 'woopress' ), '<a href="http://studiotracking.envato.com/aff_c?offer_id=4&aff_id=1564&source=DemoInstall" target="_blank">', '</a>' ); ?></p>
				...
				<table>
					<tr>
						...
						<td>
							<a href="#" class="button button-upload"><?php esc_html_e( 'Upload New Logo', 'woopress' ); ?></a>
						</td>
					</tr>
				</table>
				...
				if ( ! $this->get_default_theme_style() || count( $demo_styles ) <= 1 ) {
					...
					<p><?php esc_html_e( 'Please choose the color scheme for this website. The color scheme (along with font colors &amp; styles) can be changed at any time from the Appearance > Customize area in your dashboard.', 'woopress' ); ?></p>
					...
					<div class="theme-presets">
						<ul>
							...
							foreach ( $demo_styles as $demo_name => $demo_style ) {
								$selected = ( $demo_name == $current_demo ) ? "current" : '';
								?>
				...
				<p class="envato-setup-actions step">
					<input type="submit" class="button-primary button button-large button-next"
					       value="<?php esc_attr_e( 'Continue', 'woopress' ); ?>" name="save_step"/>
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
					   class="button button-large button-next"><?php esc_html_e( 'Skip this step', 'woopress' ); ?></a>
					<?php wp_nonce_field( 'envato-setup' ); ?>
				</p>
			</form>
			<?php
		...
		public function envato_setup_updates() {
			...
			<?php if ( etheme_is_activated() ): ?>
				...
				<?php if( get_filesystem_method() === 'direct' ) : ?>
					...
					<p class="envato-setup-actions step">
						<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
						   class="et-button button-active"><?php esc_html_e( 'Continue', 'woopress' ); ?></a>
						<?php wp_nonce_field( 'envato-setup' ); ?>
					</p>
				...
				<?php else: ?>
					...
					<div class="support-includes">
						<div>
							<p><?php esc_html_e( 'More Resources', 'woopress' ); ?></p>
							<ul>
								<li class="documentation"><a href="http://8theme.com/demo/docs/" target="_blank"><?php esc_html_e( 'Read the Theme Documentation', 'woopress' ); ?></a></li>
								<li class="howto"><a href="https://wordpress.org/support/"target="_blank"><?php esc_html_e( 'Learn how to use WordPress', 'woopress' ); ?></a></li>
								<li class="rating"><a href="http://themeforest.net/downloads"target="_blank"><?php esc_html_e( 'Leave an Item Rating', 'woopress' ); ?></a></li>
								<li class="support"><a href="https://www.8theme.com/forums/"target="_blank"><?php esc_html_e( 'Get Help and Support', 'woopress' ); ?></a></li>
							</ul>
						</div>
					...
					<p class="envato-setup-actions step">
						<a class="et-button button-active" href="<?php echo esc_url( get_admin_url() ); ?>"><?php esc_html_e( 'Go to Dashboard', 'woopress' ); ?></a>
						<a class="et-button button-active" href="<?php echo esc_url( home_url() ); ?>"><?php esc_html_e( 'View your new website!', 'woopress' ); ?></a>
						<a class="et-button" href="https://themeforest.net/user/8theme" target="_blank"><?php esc_html_e( 'Follow @8theme on ThemeForest', 'woopress' ); ?></a>
					</p>

				<?php endif;?>

			<?php else: ?>
				<h1><?php esc_html_e('Activate', 'woopress'); ?></h1>
				<form method="post">
	                <p>
	                    <label for="purchase-code"><?php esc_html_e('Enter Purchase code', 'woopress'); ?></label>
	                    <input type="text" name="purchase-code" placeholder="Example: f20b1cdd-ee2a-1c32-a146-66eafea81761" id="purchase-code" />
	                </p>
	                <p>
	                        <input class="et-button" name="etheme-purchase-code" type="submit" value="<?php esc_attr_e( 'Activate theme', 'woopress' ); ?>" />

	                </p>
	            </form>
				<p><?php _e('Use your purchase code to activate woopress template. Please, note, that you won’t be able to use it without activation.<br>A purchase code (license) is only valid for One Project. Do you want to use this theme for a one more project? Purchase a <a href="https://themeforest.net/item/woopress-responsive-ecommerce-wordpress-theme/9751050?utm_source=royalcta?utm_source=royalncta&ref=8theme&license=regular&open_purchase_for_item_id=9751050&purchasable=source">new license here</a> to get a new purchase code.', 'woopress'); ?></p>
				<h3><?php esc_html_e('To find your Purchase code', 'woopress'); ?></h3>
				<img src="<?php echo ETHEME_CODE_IMAGES_URL; ?>/purchase-code-bc.jpg">
				<br><br>
				<p><?php esc_html_e('Activate woopress template and get lifetime updates, 6 months of free top-notch support, 24/7 live support and much more.', 'woopress'); ?></p>
		...
		?>
			...
			<?php
				...
				if(isset($_REQUEST['theme_name']) && current_user_can('manage_options')){
					$this->_make_child_theme($_REQUEST['theme_name']);
				}
			...
			<p class="envato-setup-actions step">
		        <button type="submit" id= type="submit" class="et-button button-active">
		         <?php _e( 'Create and Use Child Theme', 'woopress' ); ?>
		        </button>
				<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="et-button"><?php _e( 'Skip this step', 'woopress' ); ?></a>

			</p>
			</form>
			<?php } else { ?>
			<p class="envato-setup-actions step">
				<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="et-button button-active"><?php _e( 'Continue', 'woopress' ); ?></a>
			</p>
			<?php } ?>
			<?php
		}
		...
		public function envato_setup_help_support() {
			...
			<ul class="support-blocks">
				<li>
					...
					Telegram channel<br> <a href="https://t.me/etheme" target="_blank">check</a></li>
				<li>
		...
		public function envato_setup_ready() {
			...
			<img src="<?php echo ETHEME_CODE_IMAGES_URL; ?>/success-icon-alt.png" alt="eight theme" class="elogo">
			<h1><?php esc_html_e( 'Your Website is Ready!', 'woopress' ); ?></h1>
				...
				<div>
					...
					<ul>
						<li class="documentation"><a href="http://8theme.com/demo/docs/"
						                             target="_blank"><?php esc_html_e( 'Read the Theme Documentation', 'woopress' ); ?></a>
						</li>
						<li class="howto"><a href="https://wordpress.org/support/"
						                     target="_blank"><?php esc_html_e( 'Learn how to use WordPress', 'woopress' ); ?></a>
						</li>
						<li class="rating"><a href="http://themeforest.net/downloads"
						                      target="_blank"><?php esc_html_e( 'Leave an Item Rating', 'woopress' ); ?></a></li>
						<li class="support"><a href="https://www.8theme.com/forums/"
						                       target="_blank"><?php esc_html_e( 'Get Help and Support', 'woopress' ); ?></a></li>
					</ul>
				</div>
				<div class="envato-setup-next-steps-last">
