...
class Nav_Menu_Images_Admin extends Nav_Menu_Images {
	...
	public function et_update_item_settings($menu_id, $menu_item_db_id, $args) {
		...
		foreach ($fields as $field) {
			...
			if( $field['type'] == 'checkbox') {
				if ( isset( $_REQUEST[$key] ) && is_array( $_REQUEST[$key]) && isset( $_REQUEST[$key][$menu_item_db_id] )) {
				...
				unset($value);
			} elseif ( isset( $_REQUEST[$key] ) && is_array( $_REQUEST[$key]) && isset( $_REQUEST[$key][$menu_item_db_id] )) {
				$value = $_REQUEST[$key][$menu_item_db_id];
	...
	public function enqueue_scripts() {
		...
		wp_localize_script( 'nmi-scripts', 'nmi_vars', array(
				'alert' => __( 'You need to set an image as a featured image to be able to use it as an menu item image', 'woopress' )
			)
		);
		add_thickbox();
	...
	public function _wp_post_thumbnail_html( $content, $post_id ) {
		...
		if ( has_post_thumbnail( $post->ID ) )
			...
			$link = __( 'Upload menu item image', 'woopress' );
		...
		if ( isset( $_REQUEST['thumb_was'] ) && -1 == $_REQUEST['thumb_was'] ) {
			$link_text = __( 'Change menu item image', 'woopress' );
			$ajax_nonce = wp_create_nonce( 'set_post_thumbnail-' . $post->ID );
			$remove_link = ' | <a href="#" data-id="' . $post->ID . '" class="nmi_remove" onclick="NMIRemoveThumbnail(\'' . $ajax_nonce . '\',' . $post->ID . ');return false;">' . esc_html__( 'Remove menu item image', 'woopress' ) . '</a>';
	...
	function attachment_fields_to_edit( $form_fields, $post ) {
		...
		$form_fields['nmihover'] = array(
			'label'        => __( 'Used on hover?', 'woopress' ),
			'input'        => 'html',
			...
			'value'        => $is_hover,
			'helps'        => __( 'Should this image be used on hover', 'woopress' ),
			'show_in_edit' => false
		);
		...
		$form_fields['nmiactive'] = array(
			'label'        => __( 'Used when active?', 'woopress' ),
			'input'        => 'html',
			...
			'value'        => $is_active,
			'helps'        => __( 'Should this image be used when menu item is active', 'woopress' ),
			'show_in_edit' => false
		);
