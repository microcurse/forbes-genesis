...
if ( ! empty( $product_tabs ) && $type != 'disable' ) : $i=0; ?>

    <div class="tabs <?php echo esc_attr( $type ); ?>">
        <ul class="tabs-nav">
            <?php foreach ( $product_tabs as $key => $product_tab ) : $i++; ?>
                <li>
                    <a href="#tab_<?php echo esc_attr( $key ); ?>" id="tab_<?php echo esc_attr( $key ); ?>" class="tab-title <?php if($i == 1 && $first_tab[0] != 1) echo 'opened'; ?>"><?php echo apply_filters( 'woocommerce_product_' . $key . '_tab_title', $product_tab['title'], $key ) ?></a>
                </li>
                <?php if ( $type == 'accordion' ): ?>
                    <div class="tab-content tab-<?php echo esc_attr( $key ); ?>" id="content_tab_<?php echo esc_attr( $key ); ?>" <?php if($i == 1 && $first_tab[0] != 1) echo 'style="display:block;"'; ?>>
        ...
        <?php if ( $type != 'accordion' ): ?>
            <?php $i = 0; foreach ( $product_tabs as $key => $product_tab ) : $i++; ?>
                <div
                    class="tab-content tab-<?php echo esc_attr( $key ); ?>"
                    id="content_tab_<?php echo esc_attr( $key ); ?>"
                    <?php if($i == 1 &&  isset( $first_tab[0] ) && $first_tab[0] != 1) echo 'style="display:block;"'; ?>
                >
                    <div class="tab-content-inner">
                        <?php call_user_func( $product_tab['callback'], $key, $product_tab ) ?>
                    </div>
